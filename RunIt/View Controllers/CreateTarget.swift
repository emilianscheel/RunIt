...
protocol CreateTargetDelegate {
	func didFinishedCreating(target: Target?)
}

class CreateTarget: UITableViewController {
	...
	var targets: [Target] = CoreDataStack.loadTargets()
	var currentTarget: Target!
	var location: Location?
	var context: NSManagedObjectContext?
	var delegate: CreateTargetDelegate?
	...
	override func viewDidLoad() {
		...
		context = CoreDataStack.shared.workingContext
	...
	@IBAction func backTapped(_ sender: Any) {
		delegate?.didFinishedCreating(target: nil)
		self.dismiss(animated: true, completion: nil)
	}
	
	@IBAction func saveTapped(_ sender: UIButton) {
		
		// Formatiere die Daten aus den Input-Feldern, wenn es leer ist, dann setze es auf „Unbenannt“
...
extension CreateTarget: SelectLocationDelegate {
...
extension CreateTarget: SegueHandlerType {
