...
class LastRunsDetails: UITableViewController {
	...
	override func viewDidLoad() {
		...
		objectArray.append(Objects(name: "", objects: [ObjectRowTitle(distance: currentRun.distance, date: currentRun.date!)], .title))
		...
		datas.append(ObjectRowDatas(name: "Pace min/km", value: formattedPace, iconName: "speedometer"))
		// Füge die formattierten KCal zu den ‘datas‘
		let formattedKCal = FormatDisplay.kCal.from(currentRun.duration)
		if  formattedKCal != "" {
			datas.append(ObjectRowDatas(name: "KCal", value: formattedKCal, iconName: "flame"))
		}
		
		// Füge die ‘datas‘-Section hinzu
		objectArray.append(Objects(name: "Daten", objects: datas, .data))
		...
		if targets.count != 0 {
			objectArray.append(Objects(name: "Ziele", objects: targets, .badge))
		}
		...
		objectArray.append(Objects(name: "Notizen", objects: notes, .notes))
...
extension LastRunsDetails {
	...
	override func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
		...
		if objectArray[indexPath.section].kind == .title {
			...
			// get data from objectArray with indexPath
			let data = objectArray[indexPath.section].objects[indexPath.row] as! ObjectRowDatas
